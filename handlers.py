...
class GameAuthorHandler(BaseRequestHandler):
    def get(self):
        """Handles GET /gview"""    
        return self.render('game_author.html')
...
class GamebotHandler(BaseRequestHandler):
    def get(self):
        chatinput = self.request.get('msg')
        logging.info("chat_text= " + chatinput)
        chatresponse =  augmi_chatbot.respond(chatinput)
        logging.info(chatresponse)
        self.response.out.write(chatresponse)
